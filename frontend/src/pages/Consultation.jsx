...
function Consultation({ language = "ar" }) {
  ...
  const hiddenRequestsStorageKey = useMemo(
    () => `hidden_consultation_requests_${userType}`,
    [userType]
  );
        ...
        <div className="consultation-message-box">
          ...
          <button
            ...
            style={{
              ...
              boxShadow: `0 12px 24px ${mainColor}33`,
            }}
          >
            {isArabic ? "حسناً" : "OK"}
          </button>
            ...
            requests.map((req) => {
              ...
              return (
                <div key={req.id} className="consultation-request-card">
                  <h4>
                    {isArabic ? "طلب من:" : "Request from:"} {userName}
                  </h4>

                  <p>
                    <b>{isArabic ? "وقت الجلسة:" : "Session Time:"}</b>{" "}
                    {timeSlot}
                  </p>

                  {reqTime && (
                    <p className="muted">
                      <b>{isArabic ? "وقت الإرسال:" : "Sent At:"}</b>{" "}
                      {reqTime}
                    </p>
                  )}
                  ...
                  {req.status === "pending" && (
                    <div className="action-buttons">
                      ...
                      <DeleteButton
                        onClick={() => openDeleteConfirm(req, "expert")}
                      />
                    </div>
                  )}
            ...
            myReqs.map((req) => {
              ...
              return (
                <div key={req.id} className="request-item">
                  <p>
                    <b>{isArabic ? "الخبير:" : "Expert:"}</b> {expertName}
                  </p>

                  {req.time_slot && (
                    <p>
                      <b>{isArabic ? "ميعاد الجلسة:" : "Session Time:"}</b>{" "}
                      {req.time_slot}
                    </p>
                  )}
                  ...
                  <div className="action-buttons">
                    {req.status === "accepted" ? (
                      <>
                        ...
                        <DeleteButton
                          onClick={() => openDeleteConfirm(req, "user")}
                        />
                      </>
                    ) : req.status === "pending" ? (
                      <>
                        <p className="faded">
                          {isArabic
                            ? "بانتظار موافقة الخبير"
                            : "Waiting for the expert's approval"}
                        </p>
                  ...
                  <div className="expert-info">
                    ...
                    <p>
                      {expert.specialization ||
                        (isArabic ? "خبير زراعي" : "Agricultural Expert")}
                    </p>

                    <p>
                      {isArabic ? "خبرة:" : "Experience:"}{" "}
                      {expert.experience_years ?? expert.experience ?? "-"}
                    </p>
                  </div>
                </div>
              ))}
              ...
              <div className="time-slots">
                {timeSlots.map((slot, index) => (
                  <button
                    key={index}
                    className={`time-slot ${
                      consultationTime === slot ? "selected" : ""
                    }`}
